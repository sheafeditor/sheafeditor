...
Thanks for taking a look. Sheaf is early — the core WYSIWYG editor is what ships today — so there is a lot of surface area where help is useful.
...
A pull request that introduces a parse-and-serialize round-trip will not be merged, however convenient it is. If a feature seems to need one, open an issue first and let's find another way.
...
CI runs the type-check, the tests and the package build on every pull request. Please run at least the first two before pushing.
...
- Say what user-visible behavior changed, and how you verified it. A screenshot or short clip helps for anything visual.
