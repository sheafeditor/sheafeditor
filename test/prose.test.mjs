...
// CodeMirror's measuring checks `instanceof Window`.
if (!globalThis.Window && window.Window) globalThis.Window = window.Window;
// jsdom has no layout; CodeMirror's measuring asks ranges for rectangles, so give it empty ones.
...
const { runAll } = createRequire(import.meta.url)('./prose.bundle.cjs');
...
console.log(`\n${pass}/${results.length} prose checks passed`);
